          ...
          return (
            ...
            >
              <div className="flex justify-between items-start">
                <span className={`text-[9px] font-black ${isToday ? 'text-indigo-600' : 'text-slate-300'}`}>
                  {item.day.toString().padStart(2, '0')}
                </span>
                {isToday && <div className="w-1 h-1 bg-indigo-500 rounded-full"></div>}
              </div>
              
              <div className="flex flex-col items-center">
                <span className={`text-[8px] font-black leading-none mb-1 text-center break-all ${isNegative ? 'text-rose-600' : 'text-indigo-600'}`}>
                  {item.balance !== null ? formatCompactCurrency(item.balance) : ''}
                </span>
                <div className={`w-full h-[2px] rounded-full opacity-20 ${isNegative ? 'bg-rose-500' : 'bg-indigo-500'}`}></div>
              </div>
