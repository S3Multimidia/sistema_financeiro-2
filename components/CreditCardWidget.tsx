...
export const CreditCardWidget: React.FC<CreditCardWidgetProps> = ({
    ...
    // STATE DEFINITIONS
    const [view, setView] = useState<'list' | 'add_card' | 'add_purchase' | 'details' | 'edit_purchase' | 'edit_card'>('list');
    ...
    const handleSaveEdit = () => {
        ...
        const handleEditCardStart = (card: ICreditCard) => {
            setEditingCard(card);
            setView('edit_card');
        };
        ...
        const handleDeleteCard = () => {
            if (!editingCard) return;
            if (confirm('Tem certeza que deseja excluir este cartão? Isso removerá as faturas futuras.')) {
        ...
        return (
            <div className="bg-[#1e1e2d] p-5 rounded-3xl h-full flex flex-col border border-white/5 shadow-2xl">
                                ...
                                <div key={card.id} onClick={() => handleViewDetails(card.id)} className="cursor-pointer p-4 bg-white/5 border border-white/5 rounded-2xl hover:bg-white/10 transition-all group">
                                    ...
                                    <div className="flex justify-between items-end">
                                        ...
                                        <div className="flex flex-col items-end gap-1">
                                            <button onClick={(e) => { e.stopPropagation(); handleEditCardStart(card); }} className="p-1.5 text-white/30 hover:text-white hover:bg-white/10 rounded-lg transition-colors mb-2">
                                                <Settings size={14} />
                                            </button>
                ...
                {/* EDIT CARD VIEW */}
                {view === 'edit_card' && editingCard && (
                    <div className="space-y-3 animate-fade-in bg-white/5 p-4 rounded-2xl border border-white/5">
                        <div className="flex justify-between items-center mb-2">
                            <h4 className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest">Editar Cartão</h4>
                            <button onClick={handleDeleteCard} className="text-rose-400 hover:text-rose-300 transition-colors p-1.5 hover:bg-rose-500/10 rounded-lg">
                                <Trash2 size={14} />
                            </button>
                        </div>

                        <input className="w-full bg-slate-900/50 border border-white/10 rounded-xl p-3 text-xs text-white focus:outline-none focus:border-indigo-500/50 placeholder:text-white/20" placeholder="Nome do Cartão" value={editingCard.name} onChange={e => setEditingCard({ ...editingCard, name: e.target.value })} />

                        <div className="flex gap-2">
                            <div className="flex-1">
                                <label className="text-[9px] font-bold text-white/30 ml-1 uppercase">Fechamento</label>
                                <input type="number" className="w-full bg-slate-900/50 border border-white/10 rounded-xl p-2 text-xs text-white" placeholder="Dia" value={editingCard.closingDay} onChange={e => setEditingCard({ ...editingCard, closingDay: parseInt(e.target.value) })} />
                            </div>
                            <div className="flex-1">
                                <label className="text-[9px] font-bold text-white/30 ml-1 uppercase">Vencimento</label>
                                <input type="number" className="w-full bg-slate-900/50 border border-white/10 rounded-xl p-2 text-xs text-white" placeholder="Dia" value={editingCard.dueDay} onChange={e => setEditingCard({ ...editingCard, dueDay: parseInt(e.target.value) })} />
                            </div>
                        </div>

                        <div className="space-y-1">
                            <label className="text-[9px] font-bold text-white/30 ml-1 uppercase">Limite</label>
                            <input type="number" className="w-full bg-slate-900/50 border border-white/10 rounded-xl p-2 text-xs text-white" placeholder="R$ 0,00" value={editingCard.limit} onChange={e => setEditingCard({ ...editingCard, limit: parseFloat(e.target.value) })} />
                        </div>

                        <div className="flex gap-2 mt-2">
                            <button onClick={() => { setView('list'); setEditingCard(null); }} className="flex-1 bg-white/10 hover:bg-white/20 text-white py-2 rounded-xl font-bold text-[10px] uppercase tracking-wider transition-all">Cancelar</button>
                            <button onClick={handleSaveCard} className="w-full bg-indigo-600 hover:bg-indigo-500 text-white py-3 rounded-xl font-bold text-[10px] uppercase tracking-wider shadow-lg mt-2 transition-all">Salvar Alterações</button>
                        </div>
                    </div>
                )}
